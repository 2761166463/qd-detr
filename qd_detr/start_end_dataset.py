...
class StartEndDataset(Dataset):
    ...
    def __init__(self, dset_name, data_path, v_feat_dirs, q_feat_dir,
        ...
        #        video_feat_dim=2048,  # 根据实际特征维度调整
        #        text_feat_dim=512,
        #        latent_dim=512
        #    ).to(self.device)
        #    self.ae.eval()
    ...
    def _get_query_feat_by_qid(self, qid):
        ...
        #if self.use_ae and self.apply_to_modality in ["text", "all"]:
            #if hasattr(self.ae, 'text_encoder'):
        #        if q_feat_tensor.dim() == 1:  # (D,) -> (1, D)
        #            q_feat_tensor = q_feat_tensor.unsqueeze(0)
        #        elif q_feat_tensor.dim() == 2:  # (L, D) -> (D, L) -> (1, D, L)
        #            q_feat_tensor = q_feat_tensor.permute(1, 0).unsqueeze(0)
                
        #        with torch.no_grad():
         #           text_feat = self.ae.text_encoder(q_feat_tensor)
         #       return text_feat.squeeze().cpu()
...
def start_end_collate(batch):
    ...
    for k in model_inputs_keys:
        if k in ["orig_video_feat", "orig_query_feat"]:
