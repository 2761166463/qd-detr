...
class BaseOptions(object):
    ...
    def initialize(self):
       ...
       # ========== 多模态自编码器配置 ========== 
        parser.add_argument("--use_ae", action="store_true", help="启用多模态自编码器")
        parser.add_argument("--ae_latent_dim", type=int, default=512,
                            help="自编码器潜在空间维度")
        parser.add_argument("--lw_ae", type=float, default=0.5,  
                           help="自编码器重建损失权重")
        # 新增特征破坏配置参数
        parser.add_argument("--corruption_type", type=str, default="none", 
                           choices=["none", "mask", "noise", "token_mask", "text_noise", 'time_mask', 'band_drop'], help="特征破坏类型")
        parser.add_argument("--corruption_ratio", type=float, default=0.0,
                           help="特征破坏比例，范围0.0~1.0") 
        parser.add_argument("--apply_to_modality", type=str, default="video",
                           choices=["video", "audio", "text", "all"], help="应用模态选择")
