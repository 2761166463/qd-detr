...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        if opt.a_feat_dir is None:
            ...
            if opt.use_ae:
                # 视频特征处理
                if 'video' in opt.apply_to_modality and 'video_feat' in model_inputs:
                    model_inputs["orig_video_feat"] = model_inputs["video_feat"].clone()
                # 文本特征处理
                if 'text' in opt.apply_to_modality and 'query_feat' in model_inputs:
                    model_inputs["orig_query_feat"] = model_inputs["query_feat"].clone()
        else:
            model_inputs, targets = prepare_batch_inputs_audio(batch[1], opt.device, non_blocking=opt.pin_memory)
            if opt.use_ae:
                if 'audio' in opt.apply_to_modality and 'audio_feat' in model_inputs:
                    model_inputs["orig_audio_feat"] = model_inputs["audio_feat"].clone()
        ...
        ae_loss = 0
        if opt.use_ae:
            if "video" in opt.apply_to_modality and 'video_recon' in outputs:
                ae_loss += F.mse_loss(
                    outputs['video_recon'], 
                    model_inputs["orig_video_feat"].squeeze(1)  # 去除多余的维度 [bs,1,2048] -> [bs,2048]
                )
            if "text" in opt.apply_to_modality and 'text_recon' in outputs:
                ae_loss += F.mse_loss(
                    outputs['text_recon'],
                    model_inputs["orig_query_feat"]
                )
            if "audio" in opt.apply_to_modality and 'audio_recon' in outputs:
                ae_loss += F.mse_loss(
                    outputs['audio_recon'],
                    model_inputs["orig_audio_feat"]
                ) 

        # 合并任务损失和重建损失
        task_loss = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
        losses = task_loss + opt.lw_ae * ae_loss
...
def start_training():
    ...
    if opt.a_feat_dir is None:
        dataset_config = dict(
            ...
            corruption_ratio=opt.corruption_ratio,
            apply_to_modality=opt.apply_to_modality,
        )
    ...
    if opt.eval_path is not None:
        ...
        dataset_config["corruption_type"] = opt.corruption_type
        dataset_config["corruption_ratio"] = opt.corruption_ratio
        dataset_config["apply_to_modality"] = opt.apply_to_modality
        dataset_config["q_feat_dir"] = opt.t_feat_dir.replace("sub_features", "text_features")  # for pretraining
        # dataset_config["load_labels"] = False  # uncomment to calculate eval loss
        if opt.apply_to_modality == "audio":
            eval_dataset = StartEndDataset_audio(**dataset_config)
            logger.info(f"音频模态配置 | 破坏类型: {eval_dataset.corruption_type}")
        elif opt.apply_to_modality == "text":
            eval_dataset = StartEndDataset(**dataset_config)
            logger.info(f"文本模态破坏生效 | 类型: {eval_dataset.corruption_type} 比例: {eval_dataset.corruption_ratio}")
        else:  # 默认视频模式
            eval_dataset = StartEndDataset(**dataset_config)
            logger.info(f"视频模态配置 | 破坏类型: {eval_dataset.corruption_type}")
        # 新增校验日志 ▼▼▼
        logger.info(f"验证集破坏配置 - 模态: {eval_dataset.apply_to_modality} "
                    f"类型: {eval_dataset.corruption_type} "
                    f"比例: {eval_dataset.corruption_ratio}")
    else:
        eval_dataset = None

    model, criterion, optimizer, lr_scheduler = setup_model(opt)
    if opt.use_ae:
        logger.info(f"自编码器配置 - 模态:{model.ae.apply_to_modality} 损失权重:{opt.lw_ae}")
        if not hasattr(model, 'ae'):
            raise AttributeError("模型缺少自编码器模块，请检查模型初始化参数")
