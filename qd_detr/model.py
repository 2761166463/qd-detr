...
class QDDETR(nn.Module):
    ...
    def __init__(self, transformer, position_embed, txt_position_embed, txt_dim, vid_dim,
                 ...
                 corruption_type="none", corruption_ratio=0.0, apply_to_modality="video", use_ae=False, 
                 lw_ae=0.5):
        ...
        Parameters:
            ...
            corruption_type: (新增) 破坏类型 ["none", "mask", "noise", "token_mask", "text_noise", "time_mask", "band_drop"]
            corruption_ratio: (新增) 特征破坏比例
            apply_to_modality: (新增) 应用模态 ["video", "audio", "text", "all"]
        ...
        self.apply_to_modality = apply_to_modality
        self.input_txt_proj = nn.Sequential(*[
            LinearLayer(txt_dim, hidden_dim, layer_norm=True, dropout=input_dropout, relu=relu_args[0]),  # 输入维度修正为txt_dim
        ...
        if self.use_ae:
            self.ae = MultimodalAutoencoder(
                video_feat_dim=vid_dim,  # 使用原始视频特征维度
                text_feat_dim=txt_dim,   # 使用原始文本特征维度
                audio_feat_dim=aud_dim,  # 音频特征维度
                latent_dim=256,          # 与transformer隐藏层维度匹配
                apply_to_modality=self.apply_to_modality
            )
            self.recon_loss = nn.MSELoss()

    def forward(self, src_txt, src_txt_mask, src_vid, src_vid_mask, src_aud=None, src_aud_mask=None, **kwargs):
        ...
        """
        # 保存原始未投影的特征 ▼▼▼
        orig_video_feat = kwargs['orig_video_feat']
        orig_query_feat = kwargs['orig_query_feat']
        orig_audio_feat = kwargs['orig_audio_feat']
        # 自编码器处理前保留原始特征 ▼▼▼
        if self.use_ae and hasattr(self, 'ae'):
            # 使用破坏后的特征作为输入
            ae_inputs = {
                'video': kwargs.get('video_feat', src_vid),
                'text': kwargs.get('query_feat', src_txt),
                'audio': kwargs.get('audio_feat', src_aud)
            }
            
            # 自编码器前向传播
            ae_outputs = self.ae(ae_inputs)
            
            # 计算重建损失
            ae_loss = 0
            if "video" in self.apply_to_modality:
                ae_loss += self.recon_loss(
                    ae_outputs["video_recon"], 
                    orig_video_feat
                ) 
                
            if "text" in self.apply_to_modality:
                ae_loss += self.recon_loss(
                    ae_outputs["text_recon"],
                    orig_query_feat
                ) 

            if "audio" in self.apply_to_modality and src_aud is not None:
                ae_loss += self.recon_loss(
                    ae_outputs["audio_recon"],
                    orig_audio_feat
                )

            # 使用编码后的特征替换原始输入
            if "video" in self.apply_to_modality:
                src_vid = ae_outputs["video_encoded"].unsqueeze(1)  # 添加序列维度
            if "text" in self.apply_to_modality:
                src_txt = ae_outputs["text_encoded"]
            if "audio" in self.apply_to_modality and src_aud is not None:
                src_aud = ae_outputs["audio_encoded"]
        ...
        out = {
            ...
            'video_feat': src_vid,  # 重建后的视频特征
            'query_feat': src_txt,   # 新增文本重建特征
            'audio_feat': src_aud,   # 新增音频重建特征
            'orig_video_feat': orig_video_feat,  # 原始视频特征
            'orig_query_feat': orig_query_feat,
            'orig_audio_feat': orig_audio_feat
        }
        ...
        if self.use_ae:
            out['ae_loss'] = ae_loss * self.lw_ae
        return out
...
class SetCriterion(nn.Module):
    ...
    def loss_spans(self, outputs, targets, indices):
        ...
        # 添加自编码器损失 ▼▼▼
        if 'ae_loss' in outputs:
            losses['ae_loss'] = outputs['ae_loss']
        return losses
