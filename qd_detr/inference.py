...
from qd_detr.utils.autoencoder import MultimodalAutoencoder
import torch
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    for batch in tqdm(eval_loader, desc="compute st ed scores"):
        ...
        query_meta = batch[0]
        if opt.apply_to_modality == "text":  # 新增文本破坏模式
            model_inputs, targets = prepare_batch_inputs_text(
        ...
        if opt.use_ae:  # 新增自编码器处理
            with torch.no_grad():
                ...
                if 'video' in opt.apply_to_modality and model_inputs.get("src_vid") is not None:
                    video_recon = model.ae.video_decoder(outputs['video_latent'])
                    vid_recon_loss = F.mse_loss(video_recon, model_inputs["src_vid"])
            
                if 'text' in opt.apply_to_modality and model_inputs.get("src_txt") is not None:
                    text_recon = model.ae.text_decoder(outputs['text_latent'])
                    txt_recon_loss = F.mse_loss(text_recon, model_inputs["src_txt"])
                    
                if 'audio' in opt.apply_to_modality and model_inputs.get("src_aud") is not None:
                    audio_recon = model.ae.audio_decoder(outputs['audio_latent'])
                    aud_recon_loss = F.mse_loss(audio_recon, model_inputs["src_aud"])
        ...
        if criterion:
            ...
            if opt.use_ae:
                # 添加条件赋值
                if 'video' in opt.apply_to_modality:
                    loss_dict["ae_vid_loss"] = vid_recon_loss * opt.lw_ae
                    losses += loss_dict["ae_vid_loss"]
                if 'text' in opt.apply_to_modality:
                    loss_dict["ae_txt_loss"] = txt_recon_loss * opt.lw_ae
                    losses += loss_dict["ae_txt_loss"]
                if 'audio' in opt.apply_to_modality:
                    loss_dict["ae_aud_loss"] = aud_recon_loss * opt.lw_ae 
                    losses += loss_dict["ae_aud_loss"]
            loss_dict["loss_overall"] = float(losses)  # for logging only
            for k, v in loss_dict.items():
                loss_meters[k].update(float(v) * weight_dict[k] if k in weight_dict else float(v))
...
def setup_model(opt):
    ...
    # 新增自编码器加载
    if opt.use_ae:
        model.ae = MultimodalAutoencoder(
            video_feat_dim=opt.v_feat_dim,
            text_feat_dim=opt.t_feat_dim,
            audio_feat_dim=opt.a_feat_dim,
            latent_dim=opt.ae_latent_dim,
